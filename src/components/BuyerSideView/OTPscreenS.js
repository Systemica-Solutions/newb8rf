...
import style from "./OtpScreenS.css";
...
function OTPscreenS() {
  ...
  const [responseDataBuyerNumber, setResponseDataBuyerNunber] = useState("");
  ...
  useEffect(() => {
    const fetchBoardDetails = async () => {
      ...
      try {
        ...
        const responseDataBuyerId = response.data.data.board.buyerId._id;
        console.log("buyer id", responseDataBoardData);
        setResponseDataBuyerNunber(
          response.data.data.board.buyerId.phoneNumber
        );
        setResponseDataBuyerPropertyId(response.data.data.board.propertyId);
  ...
  const handleSubmit = (event) => {
    ...
    if (phone.length !== 10) {
      ...
      if (responseDataBuyerNumber == phone) {
        try {
          axios
            .get(
        ...
        .then((response) => {
          ...
          axios
            ...
            .then((response) => {
              ...
              const phone = response.data.data.buyer.phoneNumber;
              // const tenantDetails = response.data.data.tenant.tenantDetails;
              console.log("Buyer id ", response.data.data.buyer._id);
              ...
              window.location.href = `/TenantSideViewS?buyerId=${response.data.data.buyer._id}&boardId=${boardId}`;
