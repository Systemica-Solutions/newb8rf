...
import { useBoardState } from "./boardStateS";
import { globalBuyerId } from "./TenantSideViewS";
...
function TenantSideViewCompS({ boards, boardId, boardData }) {
      ...
      {boards.map((property, index) => (
        <div key={index} className="px-[0.5rem] my-[1rem]">
          ...
          >
            {/* carousel */}
            <Link
              to={`/DetailViewS?boardId=${boardId}&propertyId=${property._id}&index=${index}`}
            >
              <Carousel
                showThumbs={false}
                showArrows={true}
...
export default TenantSideViewCompS;
